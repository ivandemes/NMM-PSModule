function New-NMMUamRepoPrivateShellLinkstorage {
    <#
.SYNOPSIS
...
.LINK
/rest-api/v1/uam/repo/private/shell/linkstorage
#>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess('/rest-api/v1/uam/repo/private/shell/linkstorage', 'POST')) {
            Invoke-NMMApiRequest -Method 'POST' -Path '/rest-api/v1/uam/repo/private/shell/linkstorage' -PathValues $pathValues -QueryValues $queryValues -Body $InputObject -BodyWasBound:$PSBoundParameters.ContainsKey('InputObject') -Connection $Connection
        }
    }
}
