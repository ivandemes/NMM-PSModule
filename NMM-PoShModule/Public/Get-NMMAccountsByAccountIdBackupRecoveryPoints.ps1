function Get-NMMAccountsByAccountIdBackupRecoveryPoints {
    <#
.SYNOPSIS
...
.EXAMPLE
Get-NMMAccountsByAccountIdBackupRecoveryPoints -Connection $connection
.OUTPUTS
System.Management.Automation.PSObject
.LINK
/rest-api/v1/accounts/{accountId}/backup/recoveryPoints
#>
    ...
    process {
        ...
        $queryValues = @{}
        if ($PSBoundParameters.ContainsKey('ProtectedItemId')) { $queryValues['protectedItemId'] = $ProtectedItemId }
        $response = Invoke-NMMApiRequest -Method 'GET' -Path '/rest-api/v1/accounts/{accountId}/backup/recoveryPoints' -PathValues $pathValues -QueryValues $queryValues -Connection $Connection
