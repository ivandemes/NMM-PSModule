function New-NMMAccountsByAccountIdUamPolicy {
    <#
.SYNOPSIS
...
.EXAMPLE
New-NMMAccountsByAccountIdUamPolicy -Connection $connection
.OUTPUTS
...
#>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess('/rest-api/v1/accounts/{accountId}/uam/policy', 'POST')) {
            Invoke-NMMApiRequest -Method 'POST' -Path '/rest-api/v1/accounts/{accountId}/uam/policy' -PathValues $pathValues -QueryValues $queryValues -Body $InputObject -BodyWasBound:$PSBoundParameters.ContainsKey('InputObject') -Connection $Connection
        }
    }
}
