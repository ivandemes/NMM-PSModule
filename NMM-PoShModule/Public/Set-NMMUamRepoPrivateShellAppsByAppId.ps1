function Set-NMMUamRepoPrivateShellAppsByAppId {
    <#
.SYNOPSIS
...
.EXAMPLE
Set-NMMUamRepoPrivateShellAppsByAppId -Connection $connection
.OUTPUTS
...
#>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess('/rest-api/v1/uam/repo/private/shell/apps/{appId}', 'PUT')) {
            Invoke-NMMApiRequest -Method 'PUT' -Path '/rest-api/v1/uam/repo/private/shell/apps/{appId}' -PathValues $pathValues -QueryValues $queryValues -Body $InputObject -BodyWasBound:$PSBoundParameters.ContainsKey('InputObject') -Connection $Connection
        }
    }
}
