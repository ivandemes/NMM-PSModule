function New-NMMAccountsByAccountIdHostPoolBySubscriptionIdByResourceGroupByPoolNameHostsByHostNameAllowsessions {
    <#
.SYNOPSIS
Activate / Deactivate host
.DESCRIPTION
## Notes
Put host into (deactivate) or out (activate) of drain mode. If host is in drain mode - this will prevent any new user sessions from being 
established. Existing sessions(including disconnected ones) will not be affected.
- AllowNewSessions: flag indicating whether the host should accept new sessions.
- AutoscaleRestrictionHours: optional parameter, exclude from autoscale for X hours. Excludes from scale in operations on activating.
Excludes from both scale in and scale out operations on deactivating.
...
.EXAMPLE
New-NMMAccountsByAccountIdHostPoolBySubscriptionIdByResourceGroupByPoolNameHostsByHostNameAllowsessions -Connection $connection
.OUTPUTS
System.Management.Automation.PSObject
.LINK
/rest-api/v1/accounts/{accountId}/host-pool/{subscriptionId}/{resourceGroup}/{poolName}/hosts/{hostName}/allowsessions
#>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess('/rest-api/v1/accounts/{accountId}/host-pool/{subscriptionId}/{resourceGroup}/{poolName}/hosts/{hostName}/allowsessions', 'POST')) {
            Invoke-NMMApiRequest -Method 'POST' -Path '/rest-api/v1/accounts/{accountId}/host-pool/{subscriptionId}/{resourceGroup}/{poolName}/hosts/{hostName}/allowsessions' -PathValues $pathValues -QueryValues $queryValues -Body $InputObject -BodyWasBound:$PSBoundParameters.ContainsKey('InputObject') -Connection $Connection
        }
    }
}
