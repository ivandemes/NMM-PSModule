function Set-NMMAccountsByAccountIdUamPolicyById {
    <#
.SYNOPSIS
...
- Targets.PersonalAvdDesktops.UserAzureIds: a list of personal desktop users. Policy will detect target hosts by assigned user.
- Targets.PersonalAvdDesktops.HostPoolIds: a list of target host pools.
...
.EXAMPLE
Set-NMMAccountsByAccountIdUamPolicyById -Connection $connection
.OUTPUTS
...
#>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess('/rest-api/v1/accounts/{accountId}/uam/policy/{id}', 'PUT')) {
            Invoke-NMMApiRequest -Method 'PUT' -Path '/rest-api/v1/accounts/{accountId}/uam/policy/{id}' -PathValues $pathValues -QueryValues $queryValues -Body $InputObject -BodyWasBound:$PSBoundParameters.ContainsKey('InputObject') -Connection $Connection
        }
    }
}
