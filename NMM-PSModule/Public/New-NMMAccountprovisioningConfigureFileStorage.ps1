function New-NMMAccountprovisioningConfigureFileStorage {
    <#
.SYNOPSIS
...
.LINK
/rest-api/v1/accountprovisioning/configureFileStorage
#>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess('/rest-api/v1/accountprovisioning/configureFileStorage', 'POST')) {
            Invoke-NMMApiRequest -Method 'POST' -Path '/rest-api/v1/accountprovisioning/configureFileStorage' -PathValues $pathValues -QueryValues $queryValues -Body $InputObject -BodyWasBound:$PSBoundParameters.ContainsKey('InputObject') -Connection $Connection
        }
    }
}
