function Set-NMMAccountsByAccountIdSchedulesByScheduleId {
    <#
.SYNOPSIS
...
.EXAMPLE
Set-NMMAccountsByAccountIdSchedulesByScheduleId -Connection $connection
.OUTPUTS
...
#>
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess('/rest-api/v1/accounts/{accountId}/schedules/{scheduleId}', 'PUT')) {
            Invoke-NMMApiRequest -Method 'PUT' -Path '/rest-api/v1/accounts/{accountId}/schedules/{scheduleId}' -PathValues $pathValues -QueryValues $queryValues -Body $InputObject -BodyWasBound:$PSBoundParameters.ContainsKey('InputObject') -Connection $Connection
        }
    }
}
